...
class ExchangeEconomyClass:
    ...
    def optimize_allocation_no_restrictions(self, scenario_name='less_restrictions', print_results=True):
        ...
        for guess in initial_guesses:
            ...
            if result.success and -result.fun > best_utility_A:
                ...
                optimal_allocation_A = tuple(result.x.round(2))
        ...
        for x1A in np.linspace(0, w1bar, N+1):
            for x2A in np.linspace(0, w2bar, N+1):
                ...
                if self.utility_A(x1A, x2A) >= initial_utility_A and self.utility_B(x1B, x2B) >= initial_utility_B:
                    ax_A.scatter(x1A, x2A, color='blue', s=1)  # Plotting eligible points on ax_A
        
        ax_A.scatter(self.par.w1A, self.par.w2A, color='red', s=15, label="Initial Endowment")
        ax_A.legend(bbox_to_anchor=(1.1, 1), loc='upper left')
        plt.show() 
    ...
    def plot_comparisons_with_pareto_improvements(self, w1bar=1.0, w2bar=1.0, N=75):
        ...
        for scenario_name, results in self.comparison_results.items():
            ...
            if 'allocations' in results and 'A' in results['allocations']:
                ...
                label = legend_labels_map.get(scenario_name, scenario_name)
                ax_A.scatter(x1A, x2A, color=color, label=label, s=20, edgecolor='black')  

        ax_A.legend(title='Scenarios', bbox_to_anchor=(1.1, 1), loc='upper left')
        plt.show()      
    ...
    def plot_utility_differences(self):
        ...
        data['Scenario'] = data['Scenario'].map(legend_labels_map).fillna(data['Scenario'])
    ...
    def plot_movement_from_endowment_to_equilibrium(self, equilibriums):
        ...
        plt.legend(handles=legend_handles, labels=['Red dots = equilibria'], fontsize='small', bbox_to_anchor=(1.5, 1.0))
